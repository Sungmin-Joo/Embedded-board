/* FPGA Buzzer Ioremap Control FILE : fpga_buzzer_driver.c AUTH : largest@huins.com */
...
ssize_t iom_buzzer_write(struct file *inode, const char *gdata, size_t length, loff_t *off_what)
{
	...
	if (copy_from_user(&value, tmp, 1)) return -EFAULT;
...
ssize_t iom_buzzer_read(struct file *inode, char *gdata, size_t length, loff_t *off_what) 
{
    ...
    value = _s_value & 0xF;
	if (copy_to_user(tmp, &value, 1)) return -EFAULT;
	return length;
}
