...
#define HCSR04_ECHO IMX_GPIO_NR(2, 4)
#define HCSR04_TRIGGER IMX_GPIO_NR(2, 5)
...
static irqreturn_t ultrasonics_echo_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	else
	{
		...
		if((after.tv_usec - before.tv_usec)/54 > 250 || (after.tv_usec - before.tv_usec)/54 < 0)
			distance = 250;
		else
			distance = (after.tv_usec - before.tv_usec)/54;
