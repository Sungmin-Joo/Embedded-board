...
const char*         lampTopic   = "iot-2/evt/lamp/fmt/json";
const char*         lampTopic_receive   = "iot-2/cmd/lamp/fmt/json";
...
void msgHandler(char* topic, byte* payload, unsigned int payloadLength) {
    if (!strcmp(responseTopic, topic)) {                                  // strcmp return 0 if both string matches
        ...
        if (!strcmp(lampTopic_receive, topic))
            handleCommand_lamp(payload, payloadLength);
        else
            handleCommand(payload, payloadLength);
    }
}

void handleCommand_lamp(byte* payload, unsigned int msgLen) {
    char temp[msgLen+1];
    ...
    if(String((const char*)d["lamp"]) == "on"){ 
        digitalWrite(RELAY, HIGH);
        Joo_iot_publish(lampTopic,"on","relay");
    } else {
        digitalWrite(RELAY, LOW);
        Joo_iot_publish(lampTopic,"off","relay");
    }
}
